...
- **自信程度**：高
- **理解的关键点**：
  - 分裂投票期间集群处于无Leader状态，客户端请求会失败
  - 随机超时机制保证最终能选出Leader
  - 新Leader如何通过nextIndex和matchIndex同步日志
  - Leader重启后（或新Leader当选），如果没有当前任期的新日志，无法更新commitIndex
...
- **理解的关键点**：
  ...
  - 区别：选举限制规则关注日志完整性，选举安全性关注唯一性

#### 4. 提交规则和图8的危险场景
- **掌握日期**：2026-01-19
- **自信程度**：高
- **理解的关键点**：
  - 提交规则的作用：防止旧任期日志被新Leader覆盖
  - 图8的危险场景：旧任期日志被新Leader覆盖，状态机不一致
  - 解决方案：只能提交当前任期的日志，旧任期日志通过当前任期日志间接提交
...
- **理解的关键点**：
  ...
  - 选举限制：新当选的Leader一定包含所有已提交的日志
  - 领导人完全特性：所有更高任期的Leader都包含相同的已提交日志
  - 状态机安全性：所有服务器应用的都是相同的日志

#### 6. 网络分区场景的处理
- **掌握日期**：2026-01-19
- **自信程度**：高
- **理解的关键点**：
  - 分区恢复后，旧Leader等待新Leader的心跳，不是主动发送
  - 新Leader发送心跳给旧Leader
  - 旧Leader收到更大的term，更新term并降级为Follower
...
- **理解的关键点**：
  - 综合考虑大小、延迟、内存
  - 根据网络环境调整配置（局域网、跨机房、跨地域）
  - 心跳携带日志需要限制大小，避免心跳延迟过大
  - 配置参数：maxBatchSize、maxBatchSizeBytes、maxBatchDelay
...
**主要主题**：基础机制综合复习
**核心收获**：
- 深入理解领导人选举和日志复制（分裂投票期间集群状态、nextIndex和matchIndex的作用）
- 准确理解日志新旧比较规则（先term后index）
- 区分选举限制规则和选举安全性
- 深入理解提交规则的作用和图8的危险场景
- 完整理解状态机安全性的证明链条
- 准确理解网络分区场景的处理（旧Leader等待新Leader的心跳）
- 准确理解批量日志同步的批次大小选择策略
- 综合复习所有已掌握的主题，准备进入实现设计阶段
...
- [x] 2026-01-19：完成实践场景分析（网络分区、节点崩溃、分裂投票）、性能考虑（选举超时配置、心跳频率设置、性能优化技巧、Read Index、Lease Read）、基础机制综合复习

### 未来的里程碑

- [ ] 2026-01-20~2026-01-26：完成实现设计
- [ ] 2026-01-27~2026-02-02：完成高级特性学习
- [ ] 2026-02-03~2026-02-09：完成实践和深化
